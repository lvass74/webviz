// @flow
//
//  Copyright (c) 2018-present, Cruise LLC
//
//  This source code is licensed under the Apache License, Version 2.0,
//  found in the LICENSE file in the root directory of this source tree.
//  You may not use this file except in compliance with the License.

import React from "react";
import { Lines, Spheres, Arrows, type Point } from "regl-worldview";

type Props = {
  points: { start: ?Point, end: ?Point },
};

const sphereSize: number = 0.3;
const lineSize: number = 0.1;

const defaultSphere: any = Object.freeze({
  type: 2,
  action: 0,
  scale: { x: sphereSize, y: sphereSize, z: 0.1 },
  color: { r: 1, g: 0.2, b: 0, a: 1 },
});

const defaultArrow: any = Object.freeze({

})
const defaultPose: any = Object.freeze({ orientation: { x: 0, y: 0, z: 0, w: 1 } });

export default function NavigationGoalMarker({ points: { start, end } }: Props) {
  const spheres = [];
  const lines = [];
  const arrows = [];
  if (start) {
    const startPoint = { ...start };

    spheres.push({
      ...defaultSphere,
      id: "_navigation_goal_start",
      pose: { position: startPoint, ...defaultPose },
    });

    if (end) {
      const endPoint = { ...end };
      lines.push({
        ...defaultSphere,
        id: "_navigation_goal_line",
        points: [start, end],
        pose: { ...defaultPose, position: { x: 0, y: 0, z: 0 } },
        scale: { x: lineSize, y: 1, z: 1 },
        type: 4,
      });

      spheres.push({
        ...defaultSphere,
        id: "_navigation_goal_end",
        pose: { position: endPoint, ...defaultPose },
      });
    }
    if (start) {
      // const start1: Point = { x: 0, y: 0, z: 0 };
      // const end1: Point = { x: 1, y: 1, z: 0 };
      arrows.push({
        id: '_navigation_goal_arrow',
        points: [start, end || start],
        pose: { ...defaultPose, position: { ...start } },
        scale: { x: lineSize, y: lineSize * 3, z: lineSize },
        color: { r: 1, g: 0.2, b: 0, a: 1 },
        type: 1
      })
    }
  }

  return (
    <>
      {/* {lines.length > 0 && <Lines>{lines}</Lines>} */}
      {/* {spheres.length > 0 && <Spheres>{spheres}</Spheres>} */}
      {arrows.length > 0 && <Arrows>{arrows}</Arrows>}
    </>
  );
}
